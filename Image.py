...
def _getdecoder(mode, decoder_name, args, extra=()):
    ...
    try:
        ...
        raise OSError("decoder not available") 
    return decoder(mode, *args + extra)
...
class Image:
    ...
    * :py:func:`~PIL.Image.frombytes`
    ...
    def tobytes(self, encoder_name="raw", *args):
        ...
        while True:
            ...
            if s:
                break
        if s < 0:
            raise RuntimeError("encoder error in tobytes")
    ...
    def tobitmap(self, name="image"):
        ...
        return b"".join(
            [
                "#define {name}_width {size}\n".format(name=name, size=self.size[0]).encode("ascii"),
                "#define {name}_height {size}\n".format(name=name, size=self.size[1]).encode("ascii"),
    ...
    def resize(self, size, resample=BICUBIC, box=None, reducing_gap=None):
        ...
        if resample not in (NEAREST, BILINEAR, BICUBIC, LANCZOS, BOX, HAMMING):
            message = "Unknown resampling filter."
    ...
    def getchannel(self, channel):
        ...
        if isinstance(channel, str):
            try:
                ...
                raise ValueError('The image has no channel ')
